...
export const enviarSolicitacao = (usuario, destinatario) => {
  return new Promise((resolve, reject) => {
    const updates = {};
    updates[`usuarios/${destinatario}/solicitacoes/${usuario}`] = { timestamp: serverTimestamp(), userId: usuario };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const snapshot = await get(
        child(dbRef, `usuarios/${usuario}/solicitacoes`)
      );
      if (snapshot.exists()) {
...
export const aceitarSolicitacao = (remetente, destinatario) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      updates[`usuarios/${destinatario}/amigos/${remetente}`] = {
        userId: remetente,
        timestamp: serverTimestamp(),
      };
      updates[`usuarios/${remetente}/amigos/${destinatario}`] = {
        userId: destinatario,
        timestamp: serverTimestamp(),
      };
