  ...
  return new Promise(async (resolve, reject) => {
    try {
      const snapshot = await get(
        child(ref(database), `publicacoes/${publicacao}`)
      );
      if (snapshot.exists()) {
        const publi = snapshot.val();
        publi.comentarios = publi.comentarios
          ? Object.values(publi.comentarios)
          : [];
        publi.curtidas = publi.curtidas ? Object.keys(publi.curtidas) : [];
        publi.comentarios = await Promise.all(
          publi.comentarios.map(async (p) => {
...
export const addCurtida = (publicacao, userId) => {
  return new Promise(async (resolve, reject) => {
    try {
      const dbRef = ref(database);
      const updates = {};
      updates[`publicacoes/${publicacao}/curtidas/${userId}`] = {
        timestamp: serverTimestamp(),
      };
...
export const addComentario = (publicacao, usuario, conteudo) => {
  return new Promise(async (resolve, reject) => {
    try {
      const dbRef = ref(database);
      const updates = {};
      const newComentKey = push(
        child(dbRef, `publicacoes/${publicacao}/comentarios/`)
      ).key;
      updates[`publicacoes/${publicacao}/comentarios/${newComentKey}`] = {
        userId: usuario,
        conteudo: conteudo,
        timestamp: serverTimestamp(),
      };
